...
#include <WiFi.h>
#include <TM1637Display.h>   // TM1637 – NOVÉ
...
// TM1637 7-seg display for gridState (NEW, piny sú voľné)
#define TM1637_CLK 18
#define TM1637_DIO 19
TM1637Display gridDisplay(TM1637_CLK, TM1637_DIO);
...
bool fetchPVData() {
  // ak nie je wifi, len vráť false – regulácia beží ďalej
  if (!wifiConnected || !dbInitialized) return false;
...
void setup() {
  ...
  // TM1637 init
  gridDisplay.setBrightness(0x0f);
  gridDisplay.showNumberDec(0, false);
  ...
  while (WiFi.status() != WL_CONNECTED && (millis() - wifiStartTime) < wifiTimeout) {
    delay(500);
    Serial.print(".");
    digitalWrite(LED2, HIGH); // Indicate wifi connection process
  }

  if (WiFi.status() == WL_CONNECTED) {
    wifiConnected = true;
    Serial.println("\nWiFi connected");
    digitalWrite(LED2, LOW); // Indicate successful wifi connection
    Serial.print("IP address: ");
    Serial.println(WiFi.localIP());
    db.begin(PROJECT_URL, API_KEY); // Only initialize Supabase if WiFi is connected
    dbInitialized = true;
  } else {
    wifiConnected = false;
    dbInitialized = false;
    Serial.println("\nWiFi connection failed - continuing without WiFi");
    digitalWrite(LED2, HIGH); // Keep LED on to indicate no WiFi
  }
  ...
  xTaskCreatePinnedToCore(
    fetchGridDataTask,  // Task function (preimenované aby sa nebilo s bool funkciou)
...
void adjustRelays(void *parameter) {
  static unsigned long lastLed3Blink = 0;
  static bool led3BlinkState = false;
  ...
  for (;;) {
    ...
    if (currentTime - lastGridCheckTime >= gridCheckInterval) {
      if (gridFetch == true) {
        ...
        // update 7-seg display s gridState
        int displayValue = gridState;
        if (displayValue > 9999) displayValue = 9999;
        if (displayValue < -999) displayValue = -999;
        gridDisplay.showNumberDec(displayValue, false);
        ...
        // gridDisplay clear pri chybe gridFetch
        gridDisplay.clear();
        ...
        if (thermostatState == LOW) {
          ...
          Serial.println("Thermostat is not demanding heat, turning off the relay.");
    ...
    // --- LED3 ALARM LOGIKA ---
    // WiFi OK + gridFetch OK => LED3 OFF
    // WiFi OK + gridFetch FAIL => LED3 ON (stála chyba meniča)
    // WiFi FAIL => LED3 BLINK (alarm WiFi)
    if (!wifiConnected) {
      if (currentTime - lastLed3Blink >= 500) {
        lastLed3Blink = currentTime;
        led3BlinkState = !led3BlinkState;
        digitalWrite(LED3, led3BlinkState ? HIGH : LOW);
      }
    } else {
      if (!gridFetch) {
        // WiFi je, ale nejde komunikácia s meničom
...
void sendDataToSupabase(void *parameter) {
  ...
  for (;;) {
    // Non-blocking kontrola WiFi
    if (WiFi.status() != WL_CONNECTED) {
      if (wifiConnected) {
        Serial.println("WiFi lost, regulation continues offline.");
      }
      wifiConnected = false;
      dbInitialized = false;

      unsigned long now = millis();
      if (now - lastReconnectAttempt > 10000) { // každých 10 s skúsiť reconnect
      ...
      // pri výpadku WiFi neuploadujeme, len čakáme kratšie
      vTaskDelay(pdMS_TO_TICKS(5000));
      continue;
    }

    // Tu sme, keď je WiFi pripojená
    if (!wifiConnected) {
    ...
    if (!dbInitialized) {
      Serial.println("Initializing Supabase after reconnect...");
      db.begin(PROJECT_URL, API_KEY);
      dbInitialized = true;
    }

    // Uploady – ak niečo zlyhá, LED2 zapneme, pri úspechu zhasneme
    if (fetchBatteryData()) {
...
void fetchGridDataTask(void *parameter) {
  for (;;) {
